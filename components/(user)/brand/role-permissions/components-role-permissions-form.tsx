...
import { useBrandRolePermissions } from "@/lib/hooks/useBrandRolePermissions"
...
// Permission types
const PERMISSION_TYPES = ["Add", "Edit", "View", "Delete", "Bulk", "Request"]
...
const ComponentsRolePermissionForm = ({ editData, onCancelEdit }: RolePermissionFormProps) => {
  ...
  const { mutate } = useBrandRolePermissions()
  const { authorities: teamAuthorities, isLoading: teamLoading } = useBrandUserRoles()
  ...
  const onSubmit = async (data: RolePermissionFormValues) => {
    ...
    const hasPermission = permissions.some((p) =>
      p.add || p.edit || p.view || p.delete || p.bulk || p.request
    )
    ...
    try {
      const endpoint = isEditMode
        ? "/api/brand/role-permissions/put"
        : "/api/brand/role-permissions/post"
  ...
  return (
    <div className="flex-1 bg-card text-card-foreground rounded-lg border p-6 h-auto">
            ...
            <div className="border rounded-lg overflow-hidden">
              ...
              {permissions.map((permission, moduleIndex) => (
                <div
                  ...
                  className="grid grid-cols-7 border-t hover:bg-muted/50"
                >

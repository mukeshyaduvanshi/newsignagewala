...
const ComponentsRolePermissionList = ({ onEdit }: RolePermissionListProps) => {
  const { authorities, isLoading, isError, mutate } = useBrandRolePermissions()
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(id)
    try {
      const response = await fetch("/api/brand/role-permissions/delete", {
        method: "DELETE",
        headers: {
  ...
  return (
    <div className="flex-1 bg-card text-card-foreground rounded-lg border p-6 min-h-[700px] h-auto overflow-hidden">
      <div className="mb-4">
        <h3 className="text-lg font-semibold">Role Permissions</h3>
        <p className="text-sm text-muted-foreground">
          Manage role permissions for team members
        </p>
      </div>
            ...
            >
              ...
              <div className="space-y-2">
                {authority.permissions
                  .filter((p) => p.add || p.edit || p.view || p.delete || p.request)
                  .map((permission, index) => {
                    ...
                    if (permission.bulk) activePermissions.push("Bulk")

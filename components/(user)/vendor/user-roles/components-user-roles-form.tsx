...
const ComponentsUserRoleForm = ({ editData, onCancelEdit }: UserRoleFormProps) => {
  ...
  const { mutate } = useVendorUserRoles()
  ...
  const onSubmit = async (data: UserRoleFormValues) => {
    ...
    try {
      if (isEditMode && editData) {
        ...
        const response = await fetch("/api/vendor/user-roles/put", {
          method: "PUT",
          headers: {
        ...
        const response = await fetch("/api/vendor/user-roles/post", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${accessToken}`,
          ...
          <FieldGroup>
            <FieldSet>
              <FieldLegend className="font-bold">
                {isEditMode ? "Edit User Role" : "User Roles"}
              </FieldLegend>
              <FieldDescription>
                {isEditMode 

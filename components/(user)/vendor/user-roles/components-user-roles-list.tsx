...
const ComponentsUserRoleList = ({ onEdit }: UserRoleListProps) => {
  const { authorities, isLoading, isError, mutate } = useVendorUserRoles()
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(id)
    try {
      const response = await fetch("/api/vendor/user-roles/delete", {
        method: "DELETE",
        headers: {
      ...
      if (!response.ok) {
        if (result.isUsedInTeam) {
          toast.error(result.error || "This user role is assigned to team members and cannot be deleted")
        } else {
      ...
      <div className="p-6 overflow-y-auto h-[450px]">
        {authorities.length > 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">No Authority Roles Found</h3>

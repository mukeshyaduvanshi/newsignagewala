...
export interface IBrandRate extends Document {
  elementName: string;
  uniqueKey?: string;
  description: string;
  rateType: string;
...
const BrandRateSchema: Schema<IBrandRate> = new Schema(
  {
    elementName: {
      type: String,
...
BrandRateSchema.index({ createdId: 1, isActive: 1 });
BrandRateSchema.index({ parentId: 1, isActive: 1 });

const BrandRate: Model<IBrandRate> =
  mongoose.models.BrandRate || mongoose.model<IBrandRate>("BrandRate", BrandRateSchema);

export default BrandRate;

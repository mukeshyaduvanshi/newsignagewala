...
export function BulkAddStoreModal({
  ...
  const checkDuplicates = async (data: StoreRecord[]) => {
    ...
    try {
      const response = await fetch("/api/manager/stores/check-duplicates", {
        method: "POST",
        headers: {
        ...
        if (record.storePincode && /^\d{6}$/.test(record.storePincode)) {
          try {
            const response = await fetch(
              `/api/manager/stores/pincode-lookup?pincode=${record.storePincode}`,
              {
                headers: {
                  Authorization: `Bearer ${accessToken}`,
                },
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/manager/stores/bulk", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${accessToken}`,

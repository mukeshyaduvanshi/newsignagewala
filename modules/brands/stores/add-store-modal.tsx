...
export function AddStoreModal({ open, onOpenChange }: AddStoreModalProps) {
  ...
  }, [selectedStateCode, selectedCountryCode, form])
  ...
  useEffect(() => {
    const fetchPincodeData = async () => {
      if (pincode && pincode.length === 6 && selectedCountry === "India") {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (data.state) {
              // Find matching state in available states
              const matchingState = availableStates.find(
                (s: any) => s.name.toLowerCase() === data.state.toLowerCase()
              )
              if (matchingState) {
                form.setValue("storeState", matchingState.name)
                setSelectedStateCode(matchingState.isoCode)
  ...
  }, [pincode, selectedCountry, form, availableStates])
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      const response = await fetch("/api/manager/stores/post", {
        method: "POST",
        headers: {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Fill in the store details below. Pincode will auto-fill state and city.
          </DialogDescription>
        </DialogHeader>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select 
                    onValueChange={(value) => {
                      field.onChange(value)
                      const state = availableStates.find((s: any) => s.name === value)
                      if (state) {

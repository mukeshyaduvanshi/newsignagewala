...
export async function POST(req: NextRequest) {
  try {
    ...
    const vendorBid = tender.biddings.find(
      (bid: any) => bid.vendorId.toString() === vendorId && bid.status === "submitted"
    );
    ...
    tender.sites.forEach((site: any) => {
      if (site.calculateUnit === "sqft" && site.measurementUnit === "feet") {
        ...
        newSubtotal += site.rate * (site.width / 12) * (site.height / 12) * site.quantity;
    ...
    const newTotal = newSubtotal + (tender.additionalChargesTotal || 0) + newTax;

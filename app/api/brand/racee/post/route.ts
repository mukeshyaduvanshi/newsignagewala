...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      "StoreIds type check:",
      storeIds.map((id: string) => typeof id),
    );
    ...
    console.log(
      "All assignments (without isActive filter):",
    ...
    if (allAssignments.length > 0) {
      console.log(
        "Sample isActive values:",
        allAssignments.slice(0, 3).map((a: any) => ({
          _id: a._id,
          isActive: a.isActive,
    ...
    console.log(
      "First 3 teamIds:",
      teamIds.slice(0, 3).map((id) => id.toString()),
    );
    ...
    console.log(
      "Step 3 - Matching TeamMembers found:",
    ...
    console.log(
      "Step 4 - Starting racee creation for",
    ...
    for (const assignment of storeAssignments) {
      ...
      const teamIdStr =
        assignment.teamId && typeof assignment.teamId === "object"
      ...
      if (!matchingTeamMemberIds.has(teamIdStr)) {
        console.log(
          "Skipping assignment - teamId not in matching list. TeamId:",
          teamIdStr,
        );
        continue;
      }
